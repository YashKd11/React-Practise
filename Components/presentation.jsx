...
function presentation() {
  return (
    <div className='w-full h-full bg-zinc-100 flex gap-5 items-center px-5 relative z-[95]'>
        <div className="h-[80%] w-1/2">
            <h1 className="text-9xl font-semibold tracking-tighter font-['gilroy']">EYE-Opening</h1>
        </div>
        <div className="h-[80%] w-1/2 relative">
            <h1 className='text-4xl font-semibold tracking-tighter uppercase font-["gilroy"]'>Presentaions</h1>
            <div className="w-full h-1/4 px-2 mt-4">
                {["S:","instagram","behance","facebook","linkdin"].map((item,index)=>(<h1 className={`text-zinc-600 text-xs underline ${index === 0 ? "pb-1":""}`}>{item}</h1>))}
            </div>
            <div className="w-full h-1/4 px-2 mt-2">
                {["L:","i202-1965 W 4th Ave","Vancouver, Canada","30 Chukarina St","Lviv, Ukraine"].map((item,index)=>(<h1 className={`text-zinc-600 text-xs underline ${index === 0 ? "pb-1":""}`}>{item}</h1>))}
            </div>
            <div className="w-full h-1/4 px-2 mt-2">
                {["E:","hello@ochi.design"].map((item,index)=>(<h1 className={`text-zinc-600 text-xs underline ${index === 0 ? "pb-1":""}`}>{item}</h1>))}
            </div>
            <div className="w-1/4 h-1/4 px-2 absolute bottom-10 right-0">
                {["M:","Home","Services","Our work","About us","Insights","Contact us"].map((item,index)=>(<h1 className={`text-zinc-600 text-xs underline ${index === 0 ? "pb-1":""}`}>{item}</h1>))}
            </div>
        </div>
        <svg className="absolute left-3 bottom-3" width="50" height="15" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg">
